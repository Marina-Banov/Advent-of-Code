...
int main() {
    ...
    paths.pop_back();
    int n_rows = borders[0].second + 1 + 2;
    int n_cols = 2 * n_rows;

    char **matrix = get_matrix(n_rows, n_cols, paths, n_rows-500);
    memset(matrix[n_rows-1], '#', n_cols*sizeof(char));
    // matrix[0][n_rows] = '+';

    int count = 0;
    pair<int,int> sand(0, n_rows);  // source_of_sand
    while (is_free(matrix, sand.first, sand.second)) {
